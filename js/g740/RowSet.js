...
// g740.RowSet - набор строк
	    dojo.declare(
			...
			null,
			{
				...
				isMark: false,				// Набор строк - хранение чужих пометок
				markOwnerName: '',			// Имя набора строк, чьи пометки хранятся
...
//		nt.markCount
//
//		nt.markRowset - описание для режима маркировки через дополнительный источник данных
//		nt.markRowset.rowsetName - имя rowset
//		nt.markRowset.fieldNodeType - имя поля для nodetype, если не задано, то все строки подходят
//		nt.markRowset.fieldMark - имя поля ссылки
//
//	fld=nt.fields[name]
//		fld.name
//		fld.type
...
//		fld.nowrap - строка без переноса при отображении в grid
//		fld.evt_onaction
//		fld.js_onaction
...
// Построить описания запросов по умолчанию
			    ...
			    getRefRowSetName: function(refId, nodeType) {
			        ...
			        if (nodeType && this.nodeTypes[nodeType]) result += '[' + nodeType + ']';
			        result += '.' + refId;
			        return result;
			    },
...
//	isNavigate	- сменилась текущая строка
			    doG740Repaint: function(params) {
					...
					if (!this.isRepaintEnabled) return true;
					...
					if (this.markOwnerName && params.isFull) {
						var objOwner=this.objForm.rowsets[this.markOwnerName];
						if (objOwner) objOwner.loadMarkFromRowset();
					}
			        return true;
			    },
				doG740SyncParentRowSet: function(params) {
					...
					if (!treeParentNode.childs) {
						var firstNode=this.objTreeStorage.getFirstChildNode(this.objTreeStorage.rootNode);
						if (treeParentNode.isEmpty && firstNode) {
							treeParentNode.isEmpty=false;
							this.objParent.doG740Repaint({isRowUpdate: true});
							return;
						}
						if (!treeParentNode.isEmpty && !firstNode && this.isEnabled) {
							treeParentNode.isEmpty=true;
							this.objParent.doG740Repaint({isRowUpdate: true});
							return;
						}
						return;
...
// Проверить, разрешено ли выполнение запроса
			    getRequestEnabled: function(requestName, requestMode) {
					...
					if (this.isFilter && requestName=='refreshrow') return true;
					var node=this.getFocusedNode();
...
// Отменить несохраненные изменения в строке				
				...
				execAutoRefresh: function() {
					...
					if (isFormEnabled && this.isInActivity) {
						this.exec({
							requestName: this.autoRefreshRequest
						});
					}
					g740.execDelay.go({
						delay: 500,
...
//	para.attr		- ассоциативный массив атрибутов, не передаваемый на сервер
			    exec: function(para) {
					...
					if (para) {
						if (para.exec) {
							para=this.objForm.prepareRequestExec(para.exec, para.attr, this.name);
							...
							if (rowsetName!=this.name) {
								if (rowsetName=='#form') {
									...
									var obj=null;
									if (rowsetName=='#focus') {
										var obj=this.objForm.getFocusedRowSet();
									}
									else {
										obj=this.objForm.rowsets[rowsetName];
									}
					...
					if (this.isFilter && (para.requestName=='refresh' || para.requestName=='refreshrow')) {
					...
					if (para.requestName=='refresh') {
						...
						if (this.isTree) {
							if (this.firstFocusPath) {
								if (!para.G740params['row.focus.path']) para.G740params['row.focus.path']=this.firstFocusPath;
							}
							this.firstFocusPath='';
							this.firstFocusId='';
						}
							...
							if (this.firstFocusId) {
								if (!para.G740params['row.focus.id']) para.G740params['row.focus.id']=this.firstFocusId;
							}
							this.firstFocusPath='';
							this.firstFocusId='';
						}
					...
					if (this.isTree && (para.requestName=='refresh' || para.requestName=='navigate')) {
						var path='';
						if (para.G740params) path=para.G740params['row.focus.path'];
						if (path) {
							var path=path.toString().split(';');
							return this.execNavigateTree(path, para.requestName=='refresh');
						}
					}
					
					if (para.requestName=='navigate') {
						var id=0;
						if (para.G740params) id=para.G740params['row.focus.id'];
						var parentNode=this.getFocusedParentNode();
						var objTreeStorage=this.getTreeStorage();
						var node=objTreeStorage.getNode(id, parentNode);
						if (node) {
							this.setFocusedNode(node);
							return true;
						}
						else {
							para.requestName='refresh';
							para.requestMode='';
						}
					}
					...
					if (r) {
						if (r.specwrite) {
							var obj=this.objForm.getFocusedRowSet();
							if (obj && obj!=this) {
								if (!obj.isFilter && obj.getExistUnsavedChanges()) {
									if (!obj.exec({requestName: 'save'})) return false;
								}
							}
						}
						...
						if (r.speclocal) {
							...
							if (r.name=='set') {
								...
								var fields=this.getFields(node);
								if (para.G740params) {
									for(var fieldName in para.G740params) {
										if (!fields[fieldName]) continue;
										this.setFieldProperty({
											fieldName: fieldName,
											value: para.G740params[fieldName]
										});
									}
								}
								return true;
						...
						if (r.name=='append' || r.name=='expand' || r.name=='copy' || r.name=='move' || r.name=='link' || r.name=='refresh' || r.name=='shift') {
							isSave=true;
						}
						...
						if (r.params) for(var name in r.params) {
							var pDef=r.params[name];
							if (!pDef) continue;
							if (!pDef.priority) continue;
							if (typeof(ppp[name])!='undefined') {
								para.G740params[name]=ppp[name];
							}
							else {
								if (typeof(para.G740params[name])!='undefined') delete para.G740params[name];
							}
						}
						...
						if (this.objForm) xmlRequest.setAttribute('form', this.objForm.controller);
...
// Возвращаем рассчитаные параметры в контексте выполнения запроса				
			    _getRequestG740params: function(requestParams) {
					...
					for (var paramName in requestParams) {
						...
						if (!g740.js_eval(objRowSet, p.js_enabled, true)) continue;
						var value='';
...
// Свернуть строку (удалив подстроки)
				...
				execNavigateTree: function(path, isCollapseBefore) {
					if (this.isObjectDestroed) return false;
					if (!this.isTree) return false;
					//if (!this.isEnabled) return false;

					var oldIsRefreshChildsEnabled=this.isRefreshChildsEnabled;
					var oldIsRepaintEnabled=this.isRepaintEnabled;
					
					var isFocusedNodeChanged=false;
					try {
						this.isRefreshChildsEnabled=false;
						this.isRepaintEnabled=false;
						if (this.getExistUnsavedChanges()) {
							if (!this.exec({requestName: 'save'})) return false;
						}
						if (isCollapseBefore) {
							if (!this.exec({requestName: 'refresh', sync:true})) return false;
						}
						var parentNode=this.objTreeStorage.rootNode;
						for (var i=0; i<path.length; i++) {
							var id=path[i];
							if (i==0 && id=='root') continue;
							if ((id==null) && (i==(path.length - 1))) break;
							var node=this.objTreeStorage.getNode(id, parentNode);
							if (!node) {
								isFocusedNodeChanged=true;
								this.setFocusedNode(parentNode);
								if (!this.exec({requestName: 'expand'})) return false;
								var node=this.objTreeStorage.getNode(id, parentNode);
								if (!node) return false;
							}
							parentNode=node;
						...
						this.isRefreshChildsEnabled=oldIsRefreshChildsEnabled;
						this.isRepaintEnabled=oldIsRepaintEnabled;
						if (isFocusedNodeChanged) {
...
//		nt.markRowset.fieldMark - имя поля ссылки
				markNode: function(node) {
					...
					if (!nt.mark[node.id]) {
						if (nt.markRowset) {
							var objMark=this.objForm.rowsets[nt.markRowset.rowsetName];
							if (!objMark) return false;
							p={};
							p[nt.markRowset.fieldMark]=node.id;
							if (nt.markRowset.fieldNodeType) {
								p[nt.markRowset.fieldNodeType]=node.nodeType;
							}
							var result=objMark.exec({
								requestName: 'append',
								G740params: p,
								sync: true
							});
							...
							if (!result) return false;
							var nodeMark=objMark.getFocusedNode();
							if (!nodeMark) return false;
							var rowMark=nodeMark.info;
							if (!rowMark) return false;
							
							if (rowMark[nt.markRowset.fieldMark+'.value']!=node.id) return false;
							if (nt.markRowset.fieldNodeType) {
								if (rowMark[nt.markRowset.fieldNodeType+'.value']!=node.nodeType) return false;
							}
							
							nt.mark[node.id]=nodeMark.id;
							nt.markCount++;
							this.doG740Repaint({
								isRowUpdate: true,
								node: node,
				...
				unmarkNode: function(node) {
					...
					if (nt.mark[node.id]) {
						if (nt.markRowset) {
							var objMark=this.objForm.rowsets[nt.markRowset.rowsetName];
							if (!objMark) return false;
							var markId=nt.mark[node.id];
							if (!objMark.setFocusedId(markId)) return false;
							if (!objMark.exec({
								requestName: 'delete',
								sync: true
				...
				loadMarkFromRowset: function() {
					...
					for(var nodeType in this.nodeTypes) {
						...
						var objMark=this.objForm.rowsets[nt.markRowset.rowsetName];
						if (!objMark) continue;
						if (objMark.isObjectDestroed) continue;
						var objMarkStorage=objMark.objTreeStorage;
						if (!objMarkStorage) continue;
						
						var mark2id={};
						for(var nodeMark=objMarkStorage.getFirstChildNode(); nodeMark; nodeMark=objMarkStorage.getNextNode(nodeMark)) {
							if (!nodeMark.id) continue;
							var rowMark=nodeMark.info;
							if (nt.markRowset.fieldNodeType) {
								if (rowMark[nt.markRowset.fieldNodeType+'.value']!=nodeType) continue;
							}
							var markId=rowMark[nt.markRowset.fieldMark+'.value'];
							if (!markId) continue;
							mark2id[markId]=nodeMark.id;
						}
						...
						for(var markId in mark2id) {
							if (!nt.mark[markId]) {
								nt.mark[markId]=mark2id[markId];
								nt.markCount++;
								isChanged=true;
							}
						}
						var lstDel=[];
						for(var markId in nt.mark) {
							if (!mark2id[markId]) lstDel.push(markId);
						}
						for(var i=0; i<lstDel.length; i++) {
							markId=lstDel[i];
							delete nt.mark[markId];
							nt.markCount--;
							isChanged=true;
						}
					...
					try {
						...
						if (responseName=='ok') {
							...
							if (r.name=='move') {
								...
								var path=this.focusedPath;
								var lstRoot=this.objTreeStorage.getChildsOrdered(this.objTreeStorage.rootNode);
								for(var i=0; i<lstRoot.length; i++) {
									this.objTreeStorage.collapseNode(lstRoot[i]);
								}
								var path=this.focusedPath;
								this.execNavigateTree(path, para.requestName=='refresh');
								this._nextFocusedNode=this.getFocusedNode();
...
// Начитать из XML описателя основные параметры набора строк
			    _buildRowSetProperty: function(xmlRowSet) {
					...
					if (g740.xml.isAttr(xmlRowSet, 'readonly') && !this.isReadOnly) {
					...
					if (g740.xml.isAttr(xmlRowSet, 'autorefresh')) {
						...
						this.autoRefreshRequest='refresh';
					}
					if (g740.xml.isAttr(xmlRowSet, 'autorefreshrow')) {
						this.autoRefreshTimeout=g740.convertor.toJavaScript(g740.xml.getAttrValue(xmlRowSet, 'autorefreshrow', '60'), 'num')*1000;
						this.autoRefreshRequest='refreshrow';
					}
					if (g740.xml.isAttr(xmlRowSet, 'ignoreerror')) {
						this.isIgnoreRequestError=g740.convertor.toJavaScript(g740.xml.getAttrValue(xmlRowSet, 'ignoreerror', '0'), 'check');
					}

					if (g740.xml.isAttr(xmlRowSet, 'row.focus.id')) {
						this.firstFocusId=g740.xml.getAttrValue(xmlRowSet, 'row.focus.id', '');
					}
					...
					if (g740.xml.isAttr(xmlRowSet,'js_onnavigate')) {
						this.js_onnavigate=g740.xml.getAttrValue(xmlRowSet, 'js_onnavigate', '');
					}
					
					if (g740.xml.isAttr(xmlRowSet, 'js_readonly')) this.js_readonly=g740.xml.getAttrValue(xmlRowSet, 'js_readonly', '');
					var xmlScripts=g740.xml.findFirstOfChild(xmlRowSet, {nodeName: 'scripts'});
					if (!g740.xml.isXmlNode(xmlScripts)) xmlScripts=xmlRowSet;
					...
					for (var indexScript=0; indexScript<lstScript.length; indexScript++) {
						...
						if (name=='readonly') this.js_readonly=g740.panels.buildScript(xmlScript);
						if (name=='onnavigate') this.js_onnavigate=g740.panels.buildScript(xmlScript);
					}
			        return true;
			    },
			    _buildSection: function(xmlSection) {
					...
					if (g740.xml.isAttr(xmlSection, 'mark.rowset') && g740.xml.isAttr(xmlSection, 'mark.rowset.fieldmark')) {
						if (!nt.markRowset)	nt.markRowset={};
						nt.markRowset.rowsetName=g740.xml.getAttrValue(xmlSection, 'mark.rowset', '');
						nt.markRowset.fieldMark=g740.xml.getAttrValue(xmlSection, 'mark.rowset.fieldmark', '');
						nt.markRowset.fieldNodeType=g740.xml.getAttrValue(xmlSection, 'mark.rowset.fieldtype', '');
					}
					if (!nt.markRowset && nodeType!='') {
						var ntDef=this.nodeTypes[''];
						if (ntDef.markRowset) {
							nt.markRowset={};
					...
					// Строим описание запросов
					{
						...
						if (nt.requests['mark'] && !nt.requests['unmarkall']) {
							var rInfo=g740.rowsetRequestInfo['unmarkall'];
							var request={
								name: 'unmarkall',
								enabled: true
							};
...
// Построить структуру полей
			    _buildField: function(xmlField, nodeType) {
					...
					if (isEnabled) {
						...
						if (g740.xml.isAttr(xmlField, 'nowrap')) fld.nowrap=g740.convertor.toJavaScript(g740.xml.getAttrValue(xmlField, 'nowrap','0'),'check');
			    ...
			    doRefreshChilds: function(isTimeout) {
					...
					if (isTimeout) {
						...
						if (
							...
							var node=this.getFocusedNode();
							if (node && node.info) {
								// Если refreshrow не обновил строку, значит строка удалена, ее делаем readonly, а дочерние оставляем заблокированными
								node.info['row.readonly']='before.refreshrow';
								if (!this.exec({
									requestName: 'refreshrow',
									sync: true
								})) result=false;
								if (node.info['row.readonly']=='before.refreshrow') {
									isRowDeleted=true;
									node.info['row.readonly']=true;
								}
							}
						}
						if (!isRowDeleted) {
...
// Основная процедура навигации по дереву или линейному набору строк
			    setFocusedPath: function(path) {
					...
					if (this.js_onnavigate) result=g740.js_eval(this, this.js_onnavigate, true);
					if (this.evt_onnavigate) this.exec({
						exec: this.evt_onnavigate
					});
...
// Дополнительные процедуры навигации				
			    ...
			    doG740Get: function(name, defValue) {
					...
					if (fieldName=='rowset') {
						...
						if (propertyName=='idlist') {
							var result='';
							var parentNode=this.getFocusedParentNode();
							if (!parentNode) parentNode=this.objTreeStorage.rootNode;
							var lst=this.objTreeStorage.getChildsOrdered(parentNode);
							for(var i=0; i<lst.length; i++) {
								if (result) result+=',';
								result+=lst[i].id;
							}
			        ...
			        if (fieldName=='row') {
			            ...
			            if (propertyName=='path') {
							if (!node) return '';
							var lst=this.objTreeStorage.getNodePath(node);
							var result=lst.join(';');
							return result;
			            }
			            if (propertyName=='id') {
			        ...
			        if (propertyName=='value' || propertyName=='oldvalue' || propertyName=='color') {
						var result=node.info[fieldName + '.' + propertyName];
						if (fld && fld.type=='ref' && result=='0') result=0;
			            return result;
			        }
			        if (propertyName=='readonly') {
...
//	para.propertyName	- value|text|oldvalue|readonly|visible|color
			    getFieldProperty: function(para) {
					...
					if (para.propertyName=='value') {
						...
						result=null;
						if (row && fld) {
							result=row[para.fieldName + '.value'];
							if (fld.type=='ref' && result=='0') result=0;
						}
					}
					...
					if (para.propertyName=='oldvalue') {
						...
						result=null;
						if (row && fld) {
							result=row[para.fieldName + '.oldvalue'];
							if (fld.type=='ref' && result=='0') result=0;
						}
					}
					...
					if (para.propertyName=='visible') {
						...
						if (!fld) result=false;
						if (result && fld) {
							if (result && typeof(fld.visible)!='undefined' && !fld.visible) result=false;
							if (result && fld.js_visible && !g740.js_eval(this, fld.js_visible, true)) result=false;
						}
						if (result && row) {
							if (result && typeof(row[para.fieldName + '.visible'])!='undefined' && !row[para.fieldName + '.visible']) result=false;
						}
					}
