...
//	para.datasource
			    constructor: function (para) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, para);
			            if (!para) para = {};
			            ...
			            if (this.isTraceEnabled) g740.trace.goEnd();
			        }
			    },
// Уничтожение экземпляра объекта
			    destroy: function () {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName);
...
//	isNavigate	- сменилась текущая строка
			    doG740Repaint: function (params) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, params);
...
// Проверить, разрешено ли выполнение запроса
			    getRequestEnabled: function (requestName, requestMode) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { requestName: requestName, requestMode: requestMode });
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
...
// Проверить наличие несохраненных изменений в текущей строке
			    getExistUnsavedChanges: function () {
			        ...
			        try {
			            ...
			            if (this.isTraceEnabled) g740.trace.goEnd('return', result);
			        }
			        return result;
			    },
...
// Возвращаем рассчитаные параметры в контексте выполнения запроса				
			    _getRequestG740params: function (requestParams) {
			        ...
			        try {
			            var result = {};
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { requestParams: requestParams });
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
...
// Обработка строки ответа: row, delete, change
			    _doResponseItem: function (xmlItem, parentNode) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { xmlItem: xmlItem, parentNode: parentNode });
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
...
// Обработка строки ответа row
			    _doResponseRow: function (xmlRow, parentNode) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { xmlRow: xmlRow, parentNode: parentNode });
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
...
// Построить все
			    build: function (xmlRowSet) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, xmlRowSet);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
...
// Начитать из XML описателя основные параметры набора строк
			    ...
			    _buildSection: function (xmlSection) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, xmlSection);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.objForm) g740.systemError(procedureName, 'errorValueUndefined', 'objForm');
			            if (this.objForm.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objForm');
...
// Построить структуру запросов
			    _buildRequest: function (xmlRequest, nodeType) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { xmlRequest: xmlRequest, nodeType: nodeType });
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.objForm) g740.systemError(procedureName, 'errorValueUndefined', 'objForm');
			            if (this.objForm.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objForm');
...
// Построить структуру полей
			    _buildField: function (xmlField, nodeType) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, xmlField);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.objForm) g740.systemError(procedureName, 'errorValueUndefined', 'objForm');
			            if (this.objForm.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objForm');
			    ...
			    _buildRef: function (fieldName, xmlRef, nodeType) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, xmlRef);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			    ...
			    _buildChange: function (fieldName, xmlChange, nodeType) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, xmlChange);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
...
// Перечитка дочерних наборов строк
			    doRefreshChilds: function (isTimeout) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, isTimeout);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            var result = true;
			            for (var name in this.childs) {
			                var objChild = this.childs[name];
...
// Вернуть родителя текущего узла
			    getFocusedParentNode: function () {
			        ...
			        try {
			            ...
			            // if (this.isTraceEnabled) g740.trace.go(this.focusedPath);
...
// Основная процедура навигации по дереву или линейному набору строк
			    setFocusedPath: function (path) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, path);
			            if (this.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.objTreeStorage) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.isEnabled) return false;
			            if (!path) path = [];
...
//	value				- новое значение
	    ...
	    dojo.declare(
			...
			null,
			{
			    ...
			    getNode: function (id, parentNode) {
			        ...
			        try {
			            if (this.isTraceEnabled && this.isTraceGet) g740.trace.goStart(procedureName, { id: id, parentNode: parentNode });
			            ...
			            if (this.isTraceEnabled && this.isTraceGet) g740.trace.goEnd('return', result);
			        }
			        return result;
			    },
			    ...
			    getNextNode: function (node) {
			        ...
			        try {
			            if (this.isTraceEnabled && this.isTraceGet) g740.trace.goStart(procedureName, node);
			            if (!this.rootNode) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.isNode(node)) g740.systemError(procedureName, 'errorIncorrectTypeOfValue', 'node');
			            var result = node.nextNode;
			        }
			        finally {
			            if (this.isTraceEnabled && this.isTraceGet) g740.trace.goEnd('return', result);
			        }
			    ...
			    cutNode: function (node) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, node);
			            if (!this.rootNode) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.isNode(node)) g740.systemError(procedureName, 'errorIncorrectTypeOfValue', 'node');
			            ...
			            if (this.isTraceEnabled) g740.trace.goEnd('return', node);
			        }
			        return true;
			    },
			    ...
			    pasteNode: function (node, parentNode, prevNode, nextNode) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { node: node, parentNode: parentNode, prevNode: prevNode, nextNode: nextNode });
			            if (!this.rootNode) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!this.isNode(node)) g740.systemError(procedureName, 'errorIncorrectTypeOfValue', 'node');
			            if (node == this.rootNode) g740.systemError(procedureName, 'errorIncorrectValue', 'node=root');
			            if (node.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'node');
			    ...
			    appendNode: function (id, parentNode, prevNode, nextNode) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { id: id, parentNode: parentNode, prevNode: prevNode, nextNode: nextNode });
			            if (!this.rootNode) g740.systemError(procedureName, 'errorAccessToDestroedObject');
			            if (!parentNode) parentNode = this.rootNode;
			            if (!this.isNode(parentNode)) g740.systemError(procedureName, 'errorIncorrectTypeOfValue', 'parentNode');
			    ...
			    removeNode: function (node) {
			        ...
			        try {
			            ...
			            if (this.isTraceEnabled) g740.trace.goEnd('return', true);
			        }
			        return true;
			    },
			    ...
			    getIndex: function (node) {
			        ...
			        try {
			            ...
			            if (this.isTraceEnabled && this.isTraceGet) g740.trace.goEnd('return', node._index);
			        }
			        return node._index;
			    },
	    ...
	    dojo.declare(
			...
			null,
			{
			    ...
			    destroy: function () {
			        var procedureName = 'g740.RowSetDataApi.destroy';
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName);
			    ...
			    getValue: function (node, fieldName, defa) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { node: node, fieldName: fieldName, defa: defa });
			            var result = defa;
			    ...
			    getValues: function (node, fieldName) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { node: node, fieldName: fieldName });
			            var result = [];
			    ...
			    containsValue: function (node, fieldName, value) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { node: node, fieldName: fieldName, value: value });
			            if (!this.objRowSet) g740.systemError(procedureName, 'errorValueUndefined', 'objRowSet');
			            if (this.objRowSet.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objRowSet');
			            if (!this.objTreeStorage) g740.systemError(procedureName, 'errorValueUndefined', 'objTreeStorage');
			            if (typeof (fieldName) != 'string') g740.systemError(procedureName, 'errorIncorrectTypeOfValue', 'fieldName');
			    ...
			    loadItem: function (args) {
			        var procedureName = 'g740.RowSetDataApi.loadItem';
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, args);
			        }
			        finally {
			            if (this.isTraceEnabled) g740.trace.goEnd();
			        }
			    ...
			    fetch: function (args) {
			        ...
			        try {
			            ...
			            if (this.isTraceEnabled) g740.trace.goEnd('return', args);
			        }
			        return args;
			    },
			    close: function () {
			    ...
			    getLabelAttributes: function (node) {
			        var procedureName = 'g740.RowSetDataApi.getLabelAttributes';
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, node);
			            var result = ['id'];
			        }
			    ...
			    newItem: function (args, parentNode) {
			        ...
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { args: args, parentNode: parentNode });
			            if (!this.objRowSet) g740.systemError(procedureName, 'errorValueUndefined', 'objRowSet');
			            if (this.objRowSet.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objRowSet');
			            if (!this.objTreeStorage) g740.systemError(procedureName, 'errorValueUndefined', 'objTreeStorage');
			            ...
			            var result = this.objRowSet.getFocusedNode();
			        }
			        finally {
			            if (this.isTraceEnabled) g740.trace.goEnd('return', result);
			        }
			    ...
			    setValue: function (node, fieldName, value) {
			        ...
			        try {
			            ...
			            //						if (typeof(value)=='string') propertyName='text';
	    ...
	    dojo.declare(
			...
			null,
			{
			    ...
			    getRoot: function (onItem) {
			        ...
			        var result = true;
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, onItem);
			            if (!this.objRowSet) g740.systemError(procedureName, 'errorValueUndefined', 'objRowSet');
			            if (this.objRowSet.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objRowSet');
			            if (!this.objTreeStorage) g740.systemError(procedureName, 'errorValueUndefined', 'objTreeStorage');
			    ...
			    getChildren: function (node, onItem) {
			        var procedureName = 'g740.RowSetTreeModelApi.mayHaveChildren';
			        try {
			            if (this.isTraceEnabled) g740.trace.goStart(procedureName, { node: node, onItem: onItem });
			            if (!this.objRowSet) g740.systemError(procedureName, 'errorValueUndefined', 'objRowSet');
			            if (this.objRowSet.isObjectDestroed) g740.systemError(procedureName, 'errorAccessToDestroedObject', 'objRowSet');
			            if (!this.objTreeStorage) g740.systemError(procedureName, 'errorValueUndefined', 'objTreeStorage');
